    ...
    if (api.targetShip && window.settings.killNpcs && api.targetBoxHash == null) {
        ...
        if ((dist > 600 && (api.lockedShip == null || api.lockedShip.id != api.targetShip.id) && $.now() - api.lastMovement > 1000)) {
            ...
            if (window.settings.circleNpc) {
                ...
                var crx = null;
                if (api.targetShip.percentOfHp > 60) {
                    crx = Eliptic(api.targetShip.position, false);
                } else {
                    crx = Eliptic(api.targetShip.position, true);
                }
...
function Eliptic(enemyPosition, cw) {


    var enemy = enemyPosition;
    var shortR = window.settings.npcCircleRadius;
    var longR = window.settings.npcCircleRadius + ((Math.random() * 5) + 1);
    var rnd = Math.floor(Math.random() * 20) + 1
    longR += rnd;
    shortR += rnd;
    let f = Math.atan2(window.hero.position.x - enemy.x, window.hero.position.y - enemy.y) + 0.5;
    let s = Math.PI / 180;
    //var rotationDirectionRnd = Math.floor(Math.random() * 1000) + 1
    //let cw = false;
    //if (rnd <= 300) {
    //    cw = true;
    //}
    ...
    x = enemy.x + longR * Math.sin(f * cross);
    y = enemy.y + shortR * Math.cos(f );
    //if (cw) {
    //    f -= s;
    //} else {
    //}
