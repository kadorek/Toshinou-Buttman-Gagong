...
function Eliptic(enemy) {
    var shortR = window.settings.npcCircleRadius;
    var longR = window.settings.npcCircleRadius + 10;
    let rnd = Math.floor(Math.random() * 7) + 1
    longR += rnd;
    let angleRadian = Math.atan2(window.hero.position.y - enemy.position.y, window.hero.position.x - enemy.position.x);
    let stepRadian =  Math.PI / 180;
    //angleRadian += s;
    angleRadian = enemy.percentOfHp > 50 ? angleRadian + stepRadian : angleRadian - stepRadian;
    var ny = enemy.position.x + (shortR * Math.sin(angleRadian));
    var nx = enemy.position.y + (longR * Math.cos(angleRadian));
    return { _x: nx, _y: ny };
}